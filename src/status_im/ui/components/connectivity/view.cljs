...
(defonce show-connected-timeout (atom nil))

(defn manage-visibility [connected? anim-opacity anim-height]
  (when @show-connected-timeout
    (swap! show-connected-timeout utils/clear-timeout))
  (if connected?
    (do (animation/start
         (animation/parallel
          [(animation/timing anim-opacity
           ...
           (animation/timing anim-height
                             {:toValue 0
                              ...
                              :duration 150
                              :easing (.-ease (animation/easing))
                              :user-native-driver true})]))
        (reset! show-connected-timeout
                (utils/set-timeout
